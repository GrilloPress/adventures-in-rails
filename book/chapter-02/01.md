...
We haven't actually added any gems but the update command also removes gems from our ```gemfile.lock``` file which records all the gems we are using, and the version of each gem.
